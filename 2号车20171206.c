...
#pragma config(Sensor, in3,    salver_angle3,  sensorAnalog)
...
{
	int senVal=SensorValue[salver_angle3];
	if(1400<senVal&&salver_speed>=0)
	{
	motor[salver_motor_6] = salver_speed;
	motor[salver_motor_7] = salver_speed;
}
else if(senVal<3375&&salver_speed<0)
{
	motor[salver_motor_6]=salver_speed;
	motor[salver_motor_7]=salver_speed;
}
...
{
	if(vexRT[Btn8R]==1) salver(-127);
	else if(vexRT[Btn8U]==1) salver(127);
	else if((vexRT[Btn8R]==0) && (vexRT[Btn8U]==0)) salver(0);
}
